...
export default function Header() {
  ...
  const items = [
    {
      label: (
        <NavLink to={`/profile/${user?.taiKhoan}`} className="text-lg">
    ...
    {
      label: (
        <NavLink to={"/login"} className="text-lg" onClick={handleSignOut}>
    ...
    if (user) {
      if (user.maLoaiNguoiDung === "GV") {
        ...
        return (
          <>
            <div className="item__avatar">
              <Dropdown menu={{ items: itemsUser }} trigger={["click"]}>
                <Avatar
                  size={48}
                  src={`https://api.dicebear.com/7.x/miniavs/svg?seed=${RANDOM_NUM}`}
                />
  ...
  return (
    <div className="h-24">
      <header className="myHeader bg-white fixed w-full z-20">
        <nav
          className="container py-2 flex items-center justify-between"
          aria-label="Global"
        >
          {/* logo */}
          <div className="flex items-center mr-15">
            <NavLink to="/" className="-m-1.5 p-1.5 mr-10">
              <img className="w-52" src="../../img/logo.png" alt="logo-edu" />
            </NavLink>
            {/* Menu */}
            <Popover.Group className="hidden lg:flex gap-x-12 justify-between">
              ...
              <a href="#courseList" className="text-lg font-semibold colorText">
                Khoá học
              </a>
              <a href="#url" className="text-lg font-semibold colorText">
                Blog
              </a>
            </Popover.Group>
          </div>
          ...
          {/* Login & Search */}
          <div className="myHeader__item flex lg:flex justify-between items-center gap-x-8">
            <div className="hidden lg:flex relative items-center">
              <input
                className="search__style h-10 w-64 pl-2 rounded"
                type="text"
        ...
        <Dialog
          as="div"
          className="myHeader__mobile hidden lg:block"
          open={mobileMenuOpen}
          onClose={setMobileMenuOpen}
        >
          <div className="fixed inset-0 z-10 mr-5" />
          <Dialog.Panel className="fixed w-5/12 sm:w-full inset-y-0 right-0 z-30 overflow-y-auto bg-white px-6 py-6 ring-1 ring-gray-900/10">
            ...
            <div className="flex items-center justify-between">
              <NavLink to="/" className="-m-1.5 p-1.5">
                <img className="w-44" src="../../img/logo.png" alt="logo-edu" />
              </NavLink>
              <button
                type="button"
            ...
            <div className="mobile__item flex relative items-center mt-10">
              <input
                ...
                placeholder="Tìm kiếm..."
                onKeyDown={handleSearch}
                className="item__searchStyle pl-2 w-full h-10 rounded"
              />
              <MagnifyingGlassIcon className="absolute right-2 h-5 w-5 text-black" />
            </div>

            {/* menu for mobile */}
            <div className="mt-6 flow-root justify-between">
              <div className="-my-6 divide-y divide-gray-500/10">
                <div className="space-y-2 py-6 mr-5">
                ...
                {/* Login button */}
                <div className="py-6">{renderUserAccount()}</div>
              </div>
            </div>
          </Dialog.Panel>
        </Dialog>
